import os
# বটের কোড রান হওয়ার সাথে সাথেই pymongo ইন্সটল করে নেবে
...
def normalize_channel(channel):
    ...
    # যদি numeric id হয় (negative বা positive)
    try:
        return int(channel)
    except ValueError:
        pass
    # যদি username হয়, @ যোগ করো যদি না থাকে
...
def check_force_join(user_id):
    ...
    for channel in channels:
        try:
            ...
            # এই status গুলো valid member হিসেবে গণ্য করা হবে
            if chat_member.status in ['left', 'kicked', 'restricted']:
                not_joined.append(channel)
        except telebot.apihelper.ApiTelegramException as e:
            err_msg = str(e).lower()
            # "user not found" বা "member list is inaccessible" হলে joined ধরো না
            if 'user not found' in err_msg or 'member list is inaccessible' in err_msg:
                ...
                # চ্যানেল পাওয়া যাচ্ছে না — জয়েন মেসেজ দেখাও
                not_joined.append(channel)
            else:
                # অন্য error — assume not joined
                not_joined.append(channel)
        except Exception as e:
...
def send_force_join_message(chat_id, missing_channels):
    ...
    for ch in missing_channels:
        ...
        if ch_str.startswith('@'):
            ...
            # numeric chat id — invite link বানানো যাবে না, তাই skip
            link = None
        else:
            link = f"https://t.me/{ch_str.replace('@', '')}"
...
def check_sms_for_number(phone_number, chat_id, fetch_date=None):
    ...
    # নম্বর যে দিন নেওয়া হয়েছে সেই তারিখ সেভ করো
    if fetch_date is None:
        fetch_date = today_str()

    # ৩ সেকেন্ড করে ১৫ মিনিট (৩০০ বার)
    max_attempts = 300
    ...
    for attempt in range(max_attempts):
        ...
        # প্রথমে নম্বর নেওয়ার তারিখে খোঁজো
        sms_text = search_in_date(fetch_date)

        # যদি না পাওয়া যায় এবং আজকের তারিখ আলাদা (রাত ১২টার পর ডেট চেঞ্জ হলে)
...
def process_and_send_two_numbers(chat_id, user_range, srv_name="CUSTOM", cnt_name="Unknown"):
    ...
    # দুটি নম্বর একসাথে fetch করো (parallel)
    results = [None, None]
    ...
    # শেষ নম্বর save করো (single number flow এর জন্য)
    user_current_range[chat_id] = user_range
    if clean_nums:
        user_last_number[chat_id] = clean_nums[-1]

    # প্রতিটি নম্বরের জন্য copy button
    ...
    # উভয় নম্বরের জন্য OTP ট্র্যাকিং শুরু
    fetch_date = today_str()
...
def show_admin_panel(chat_id, message_id=None):
    ...
    # সাব-এডমিন ম্যানেজমেন্ট শুধু মেইন এডমিন দেখবে
...
def show_otp_history(chat_id, message_id=None):
    ...
    # বাংলাদেশ টাইম অনুযায়ী আজ থেকে ৭ দিন পিছনে
...
def process_add_fjc(message, msg_id):
    ...
    # @ যোগ করো যদি না থাকে এবং numeric না হয়
    try:
        int(raw)
        channel_entry = raw
    except ValueError:
...
def send_welcome(message):
    ...
    # Group চ্যাটে /start হলে keyboard remove করে reply দাও
...
def handle_text_messages(message):
    ...
    # ============================================================
    # গ্রুপ/সুপারগ্রুপে বট কে মেসেজ করলে keyboard দেখাবে না
    # OTP Group এ বট থাকলে সেখানে কোনো reply keyboard বা অ্যাকশন নেই
    # ============================================================
    if message.chat.type in ['group', 'supergroup']:
        # Group এ কোনো reply/action করবো না — শুধু ignore
        return
...
def handle_query(call):
    ...
    # --- ফোর্স জয়েন স্ট্যাটাস চেক (ফিক্সড) ---
    if call.data == "check_join_status":
        # Telegram API caching এড়াতে background thread-এ চেক করো
        def do_join_check():
            time.sleep(1)
            is_joined, missing = check_force_join(chat_id)
            if is_joined:
    ...
    # ===== SUB-ADMIN MANAGEMENT (শুধু মেইন এডমিন) =====
